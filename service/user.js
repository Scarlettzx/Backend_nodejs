...
module.exports = {
  ...
  updateUser: (data, updateAt, callBack) => {
    pool.query(
      `UPDATE users SET user_email = ?, user_name = ?, user_password = ?, user_updateAt = ? WHERE user_id = ? `,
      [data.email, data.name, data.password, updateAt, data.user_id],
      (error, results, fields) => {
        if (error) {
          callBack(error);
        }
